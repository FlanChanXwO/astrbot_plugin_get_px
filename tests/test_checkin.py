...
class CheckinStoreTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_empty_ai_record_can_be_filled_by_local(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = FrozenCheckinStore(tmp)
            await store.checkin(user_id="10001", username="tester", bot_name="neko")
            with closing(sqlite3.connect(Path(tmp) / "checkin.sqlite3")) as conn:
    ...
    async def test_nonempty_ai_record_cannot_be_overwritten_by_local(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = FrozenCheckinStore(tmp)
            await store.checkin(user_id="10001", username="tester", bot_name="neko")
            with closing(sqlite3.connect(Path(tmp) / "checkin.sqlite3")) as conn:
                conn.execute(
                    """
                    UPDATE checkin_records
                    SET greeting_source = 'ai', greeting = 'AI generated'
                    WHERE user_id = ? AND date_key = ?
                    """,
                    ("10001", "2026-05-26"),
                )
            ...
            self.assertEqual(result.greeting, "AI generated")
